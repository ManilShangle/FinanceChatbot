...
def get_predicates():
    ...
    for index, row in data.iterrows():
        context += 'name(' + str(index) + '). '
        for attr in attrs:
            if attr == 'open hours':
                context += 'name' + '(' + str(index) + ',\'' + attr + '\',\'' + str(row[attr]) + '\'). '
                ...
                values = values.split('*/*')
                for value in values:
                    context += 'name' + '(' + str(index) + ',\'' + attr + '\',\'' + value + '\'). '
                    context_list.append([attr, index, row[attr]])
        context += '\n'
...
def match_predicates(attr:str, values:list, name_list:list):
    ...
    elif 'query' in attr or attr == 'another_option' or attr == 'view_history':
...
def parse_predicates(inputs:str, attrs, values):
    ...
    close_domain = ['income level', 'risk level', 'sector']
