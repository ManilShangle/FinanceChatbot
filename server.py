    ...
    while session_continues:
        ...
        if not reply_predicates:
            ...
            if mode == 'quit':
                ...
                irr_reply = 'Sorry, I am just a finance proffesional giving stock recommendations. For other things, please ask other people for help.'
                if 'Question' in reply_predicates:
                    reply = irr_reply + ' Do you have any preference for the ' + reply_predicates['Question'] + ' of the stock?'
                    mode = 'ask'
                else:
                ...
                if 'Question' in reply_predicates:
                    reply = thank_reply + ' Do you have any preference for the ' + reply_predicates['Question'] + ' of the stock?'
                    mode = 'ask'
                else:
                    reply = thank_reply + ' What can I help you with?'
            elif mode == 'fail':
                    reply = 'Sorry, but we can\'t find the result for your requirement. We can find result with ' + reply_predicates['Results'] + ', but no stock has ' + reply_predicates['Value'] + '.'
                    mode = 'recommend'
            else:
                ...
                if mode == 'ask':
                    reply = 'Do you have any preference for the ' + reply_predicates['Value'] + ' of the stock?'

                if mode == 'view_history':
                    reply = sentence_gen(reply_predicates['Value'][1:-1]).strip()
                
                if mode == 'recommend':
                    reply = sentence_gen(reply_predicates['Value'][1:-1]).strip() + ' How does that sound?'
